...
function tournamentWinner(competitions, results) {
  const teamPoints = {}
  const highestPoint = [-Infinity, '']

  for (let i = 0; i < results.length; i++) {
    let homeAway = results[i] === 0 ? 1 : 0
    teamPoints[competitions[i][homeAway]] =
      (teamPoints[competitions[i][homeAway]] || 0) + 3
    if (teamPoints[competitions[i][homeAway]] > highestPoint[0]) {
      highestPoint[0] = teamPoints[competitions[i][homeAway]]
      highestPoint[1] = competitions[i][homeAway]
    }
  }
